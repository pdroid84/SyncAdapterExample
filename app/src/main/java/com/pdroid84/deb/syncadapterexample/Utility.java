...
public class Utility {
    ...
    public static String getArtUrlForWeatherCondition(Context context, int weatherId) {
        ...
        String artUri = sPref.getString(context.getString(R.string.pref_art_pack_key),
                context.getString(R.string.pref_art_pack_label_deb));
        // Based on weather code data found at:
        // http://bugs.openweathermap.org/projects/api/wiki/Weather_Condition_Codes
        if (weatherId >= 200 && weatherId <= 232) {
            return String.format(Locale.US, artUri, "storm");
        } else if (weatherId >= 300 && weatherId <= 321) {
            return String.format(Locale.US, artUri, "light_rain");
        } else if (weatherId >= 500 && weatherId <= 504) {
            return String.format(Locale.US, artUri, "rain");
        } else if (weatherId == 511) {
            return String.format(Locale.US, artUri, "snow");
        } else if (weatherId >= 520 && weatherId <= 531) {
            return String.format(Locale.US, artUri, "rain");
        } else if (weatherId >= 600 && weatherId <= 622) {
            return String.format(Locale.US, artUri, "snow");
        } else if (weatherId >= 701 && weatherId <= 761) {
            return String.format(Locale.US, artUri, "fog");
        } else if (weatherId == 761 || weatherId == 781) {
            return String.format(Locale.US, artUri, "storm");
        } else if (weatherId == 800) {
            return String.format(Locale.US, artUri, "clear");
        } else if (weatherId == 801) {
            return String.format(Locale.US, artUri, "light_clouds");
        } else if (weatherId >= 802 && weatherId <= 804) {
            return String.format(Locale.US, artUri, "clouds");
        }
        return null;
    }
}
